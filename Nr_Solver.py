...
* Added --max_m_expo (exponent of 10) for provably correct early rejection:
  - --max_m_expo 27 sets max_m to 10^27
...
Version 18, 19 changes (May 26, 2026)  
---------------------------------
* Changed --max_m to --max_m_expo to take the exponent of 10 for max_m
  instead of a 1 followed by a long string of zeros.
* Added last found smooth pair to JSON output so those are still
  recorded even if end_omega is past the full reporting limit.


By Ken Clements, initial version Feb 16, 2026.
"""
...
program_name, program_version = "Nr_Solver", 19
...
def compute_and_verify_lightweight(
    ...
    compute_stats: Dict[str, object] = {
        ...
        "S_raw_count": 0,
        "report_mode": "summary",
        "last_smooth_pair_m": -1,    # largest m seen (both m and m+1 P-smooth)
        "last_smooth_pair_m1": -1,   # m+1 for that pair
    }
    ...
    def _verify_one(m: int) -> None:
        ...
        # --- NEW: track largest smooth pair seen ---
        if m > compute_stats["last_smooth_pair_m"]:
            compute_stats["last_smooth_pair_m"] = m
            compute_stats["last_smooth_pair_m1"] = m + 1
        # --- end new ---
...
def write_summary_json(
    ...
    summary = {
        ...
        "last_smooth_pair_m":  compute_stats.get("last_smooth_pair_m", -1),
        "last_smooth_pair_m1": compute_stats.get("last_smooth_pair_m1", -1),
    }
    with open(path, "w", encoding="utf-8") as f:
